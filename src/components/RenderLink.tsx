...
// import { InternalLinkType, LanguageCode } from "@/constants/Types";
...
type ParsedInternal = { type: "questionLink"; identifier: number } | null;
...
const RenderLink = ({
  ...
  const handlePress = useCallback(() => {
    ...
    handleOpenInternalUrl(
      String(parsed.identifier),
      lang,
      parsed.type as InternalLinkType,
    );
  }, [url, isExternal, lang]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const q = await getQuestionInternalURL(parsed.identifier, lang);

        if (!cancelled) {
          setLabel(q?.title || `Frage ${parsed.identifier}`);
        }
      } catch (error) {
        console.error("RenderLink: failed to hydrate label", error);
